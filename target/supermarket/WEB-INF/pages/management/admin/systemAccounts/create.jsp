    ...
    <div class="col-md-12">
        <div class="panel panel-default">
            <div class="panel-heading">
                <h4>${empty account ? '新建账号':"修改账号信息"}</h4>
            </div>
            <div class="panel-body">
                <form id="systemAccount_form" class="form-horizontal" action="#" method="post">
                    ...
                    <div class="alert alert-danger ${empty account? '':'hidden'}">
                        <i class="ace-icon fa fa-exclamation-triangle red bigger-130"></i>
                        <button class="close" data-dismiss="alert">
                            <i class="ace-icon fa fa-times"></i>
                        </button>
                        注意:账号创建成功之后的默认密码为:<span class="badge badge-danger">123456</span>,登录后请立即修改密码!
                    </div>
                    <hr/>
                    <div class="form-group">
...
<script>
    $("#systemAccount_form").submit(function () {
        ...
        if(form.account.value.trim().length > 0 && form.account.value.trim().length < 16){
            if (form.realName.value.trim().length > 0 && form.realName.value.trim().length < 10 ) {
                var params = {
                    id: form.id.value,
                    account:form.account.value,
                    realName: form.realName.value
                };
                var url =  $.ctx + "/account/iframe/save";
                if (form.id.value != ""){
                    var url =  $.ctx + "/account/iframe/edit";
                    params._method = 'put';
                }
                $.ajax({
                    url:url,
                ...
                parent.layer.msg('请如实填写真实姓名', {icon: 5, time: 2000});
            }
        }else {
            parent.layer.msg('请填写正确的账号信息', {icon: 5, time: 2000});
        }
        return false;
    })
</script>
