...
function loadTable() {
	$('#table').bootstrapTable({
		...
		columns: [{
			...
			formatter: function (value, row, index) {
				var html ="";
				html = '<span class="label label-info">' + value.name + '</span> ';
				return html;
			}
		}, {
			field: 'member',
			...
			formatter: function (value, row, index) {
				var html ="";
				html = '<span class="label label-info">' + value.number + '</span> ';
				return html;
			}
		}, {
			field: 'address',
			...
			formatter: function (value, row, index) {
				if (value == 0){
					return  '<span class="label label-info">退货中</span>'
				}else if (value == 1){
			...
			formatter: function (value, row, index) {
				if (value == false) {
					return  '<span class="label label-danger">未配送</span>';
				}
				return  '<span class="label label-success">已配送</span>';
			}
		},{
			field: '',
			title: '操作',
...
function info(id) {
	layer.open({
		...
		area : ['500px' , '520px'],
		content: $.ctx + '/role/iframe/info/' + id
	});
}
...
function edit(id) {
	//默认是新增
	var url = $.ctx + '/role/iframe/edit';
	var title = "新增";
	if (id != null && id != undefined){
		//修改
		url = $.ctx + '/role/iframe/edit/' + id;
		title = "修改";
	}
	layer.open({
		type: 2,
